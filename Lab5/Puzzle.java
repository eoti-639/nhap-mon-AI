...
public class Puzzle {
	...
	public int computeH1(Node currentState) {
		...
		for(int i = 0; i < MAX_ROW; i++) {
			for(int j = 0; j < MAX_COL; j++) {
				int tileCurent = currentState.getTile(i, j);
				int tileGoal = goalState.getTile(i, j);
				if(tileCurent != 0 && (tileCurent != tileGoal))
					output++;
			}
		}
		return output;
	...
	public int computeH2(Node currentState) {
		...
		for(int i = 1; i < MAX_ROW * MAX_COL; i++) {
			int[] curentPos = currentState.getLocation(i);
			int[] goalPos = goalState.getLocation(i);
			result += PuzzleUtils.manhattanDistance(curentPos, goalPos);
		}
		return result;
	}
